@extends('layouts.app')
@section('title', config('app.name').' | '.__('_phone.pages.create.titlePage'))
@section('content')
    <div class="sections-report-phony">
        <div class="section-report-phony padding-20 margin-0-auto">
            <form method="POST" action="{{ route('phone.create.post') }}" class="form-report-phony max-width-727p flex flex-fw-w flex-jc-c padding-10 radius-5">
                ...
                <div class="flex flex-jc-sb flex-ai-c col-100 padding-10">
                    <div class="col-100 flex">
                        <button class="btn btn-primary radius-5 col-50" type="submit" name="submit" value="ReportPhoney">{{__('_app.text.create')}}</button>
                        <a href="" class="btn btn-black radius-5 col-25 margin-0-10" >{{__('_app.text.cancel')}}</a>
                    </div>
                </div>
            </form>
        </div>
